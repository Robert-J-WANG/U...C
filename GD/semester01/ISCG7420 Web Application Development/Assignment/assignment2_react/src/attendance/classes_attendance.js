...
function ClassesAttendance(props) {
    ...
    useEffect(() => {
        if (localStorage.getItem("token")){
            ...
            axios.get(BaseURL +'student_enrolment/', {
                headers: {
                    Authorization: "Token "+localStorage.getItem("token")
                }
            }).then(response => {
    ...
    return (
        <div className={'container'}>
            <h1 className={'display-6'} style={{margin: "30px"}}>Attendance - Classes</h1>
                ...
                <tbody>
                {
                    classState.loading ? classState.classes.map(class1 => {
                        const navigateToStudentList = () => {
                            navigate('/attendance/students', { state: {classID: class1.id, studentList: studentState.students }})
                        }
